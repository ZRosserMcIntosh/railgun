...
// Mock crypto object that tests can manipulate
const mockCrypto = {
  isInitialized: vi.fn(() => true),
  init: vi.fn(),
  setLocalUserId: vi.fn(),
...
// Simple messaging service mock
let messagingServiceInstance: {
  initialize: (userId: string) => Promise<void>;
  sendDm: (recipientId: string, content: string) => Promise<unknown>;
  sendChannelMessage: (channelId: string, content: string) => Promise<unknown>;
  decryptMessage: (message: unknown) => Promise<string>;
} | null = null;

function getMessagingService() {
  if (!messagingServiceInstance) {
    messagingServiceInstance = {
      initialize: async (userId: string) => {
        mockCrypto.setLocalUserId(userId);
        await mockApi.registerDeviceKeys();
      },
      sendDm: async (recipientId: string, content: string) => {
        const encrypted = mockCrypto.encryptDm(recipientId, content);
        return mockApi.sendDm(recipientId, encrypted);
      },
      sendChannelMessage: async (channelId: string, content: string) => {
        const encrypted = mockCrypto.encryptChannel(channelId, content);
        return mockApi.sendMessage(channelId, encrypted);
      },
      decryptMessage: async (message: unknown) => {
        return mockCrypto.decryptDm('sender', message as any);
      },
    };
  }
...
function resetMessagingService() {
